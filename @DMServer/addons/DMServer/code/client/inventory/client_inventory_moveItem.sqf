...
private _addThisContainer = {
	...
	if (_container isKindOf "Bag_Base" AND !(_container isEqualTo backpackContainer player)) then {_container = backpackContainer player};
	switch (true) do {
		case (_category in ["Weapon"]): {_container addWeaponWithAttachmentsCargoGlobal [_data,_amount]};			
		case ((_category in ["Item"]) OR (_type in ["Glasses","Vest","Uniform","Headgear"])): {_container addItemCargoGlobal [_classname,_amount]};			
		case (_category in ["Magazine","Mine"]): {
			if ((_data select 1) == 0) exitWith {};
			_container addMagazineAmmoCargo [_classname,_amount,_data select 1]
		};
		case (_type in ["Backpack"]): {_container addBackpackCargoGlobal [_classname call BIS_fnc_basicBackpack,_amount]};
	};
...
private _addThisLoadout = {
	...
	private _swapGear = {
		...
		private _currentMass = switch (true) do {
			...
			case (isClass (configFile >> _currentConfig >> _currentClass >> "WeaponSlotsInfo")): {getNumber (configFile >> _currentConfig >> _currentClass >> "WeaponSlotsInfo" >> "mass")};
			case (_currentConfig isEqualTo "CfgGlasses"): {getNumber (configFile >> _currentConfig >> _currentClass  >> "mass")};
			default {getNumber (configFile >> _currentConfig >>_currentClass >> "ItemInfo" >> "mass")};
		};
